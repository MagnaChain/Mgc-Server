...
Storage.prototype.storeTwoStepCache = function(walletId, cacheStatus, cb) {
  log.debug("############### Storage.prototype.storeTwoStepCache walletId: " + walletId);
  log.debug("############### Storage.prototype.storeTwoStepCache cacheStatus: " + JSON.stringify(cacheStatus) );
  var self = this;
...
Storage.prototype.getTwoStepCache = function(walletId, cb) {
  ...
  self.db.collection(collections.CACHE).findOne({
    ...
    if (err) 
	{
	  log.debug("############### Storage.prototype.getTwoStepCache err: " + err );
	  return cb(err);
	}
    if (!result) 
	{
	  log.debug("############### Storage.prototype.getTwoStepCache result is null");
	  return cb();
	}
	log.debug("############### Storage.prototype.getTwoStepCache result: " + JSON.stringify(result) );
    return cb(null, result);
  });
};
