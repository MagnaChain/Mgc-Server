...
A Multisig HD MagnaChain Wallet Server.

# Description

MagnaChain Wallet Service facilitates multisig HD wallets creation and operation through a (hopefully) simple and intuitive REST API.

MWS can usually be installed within minutes and accommodates all the needed infrastructure for peers in a multisig wallet to communicate and operate – with minimum server trust.
  

# Getting Started
```
 git clone https://github.com/MagnaChain/Mgc-Server.git
 cd Mgc-Server && npm start
```


This will launch the MWS service (with default settings) at `http://localhost:3232/bws/api`.

MWS needs mongoDB. You can configure the connection at `config.js`

MWS supports SSL and Clustering. 

MWS uses by default a Request Rate Limitation to CreateWallet endpoint. If you need to modify it, check defaults.js' `Defaults.RateLimit`

# Using MWS with PM2

MWS can be used with PM2 with the provided `app.js` script: 
 
```
  pm2 start app.js --name "Mgc-Server"
```

# Security Considerations
 * Private keys are never sent to MWS. Copayers store them locally.
 * Extended public keys are stored on MWS. This allows MWS to easily check wallet balance, send offline notifications to copayers, etc.
 ...
 * All MWS responses are verified:
  ...
  * TX Proposals templates are signed by copayers and verified by others, so the MWS cannot create or tamper with them.
